...
Configuration:
  ...
  If no config file exists, a default one will be created on first run.
    ...
    location_group = parser.add_argument_group('Location Options',
        'Specify location using either ZIP code OR coordinates (not both)')
    ...
    location_group.add_argument(
        '--latitude',
        type=float,
        help='Latitude in decimal format (range: -90 to 90)'
    )

    location_group.add_argument(
        '--longitude',
        type=float,
        help='Longitude in decimal format (range: -180 to 180)'
    )
...
def validate_cli_arguments(args: argparse.Namespace) -> list[str]:
    ...
    zipcode, coordinates, forecast_day, and forecast_hours.
    ...
    has_zipcode = args.zipcode is not None
    has_latitude = args.latitude is not None
    has_longitude = args.longitude is not None

    if has_zipcode and (has_latitude or has_longitude):
        errors.append("Cannot specify both zipcode and coordinates. Use one or the other.")
    ...
    if has_latitude or has_longitude:
        if has_latitude and not has_longitude:
            errors.append("Both latitude and longitude must be provided together")
        elif has_longitude and not has_latitude:
            errors.append("Both latitude and longitude must be provided together")
        else:
...
def main() -> int:
    ...
    if config.zipcode:
        logger.debug(f"Location: ZIP {config.zipcode}, Hours: {config.forecast_hours}, Day: {config.forecast_day}")
    else:
        logger.debug(f"Location: ({config.latitude}, {config.longitude}), Hours: {config.forecast_hours}, Day: {config.forecast_day}")
    ...
    try:
        if config.zipcode:
            logger.debug(f"Fetching current weather for ZIP code {config.zipcode}")
        else:
            logger.debug(f"Fetching current weather for coordinates ({config.latitude}, {config.longitude})")
