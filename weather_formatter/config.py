...
class WeatherConfig:
    ...
    Attributes:
        zipcode: ZIP code for weather lookup (optional if latitude/longitude provided)
    ...
    def validate(self) -> List[str]:
        ...
        # Check that either zipcode OR coordinates are provided
        has_zipcode = bool(self.zipcode)
        has_coordinates = self.latitude is not None and self.longitude is not None

        if not has_zipcode and not has_coordinates:
            errors.append("Either zipcode OR latitude/longitude coordinates are required")
        elif has_zipcode and has_coordinates:
            errors.append("Provide either zipcode OR latitude/longitude coordinates, not both")
